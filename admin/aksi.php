...
//echo $sql;
   ...
   while ($simpan = mysql_fetch_array($dataBarang)) {
      ...
      $sql_simpan = "INSERT INTO detail_beli(idTransaksiBeli,barcode,
                        tglExpire,qty,jumBarangAsli,hargaBeli,username,kd_brg,barang_id)
                    VALUES('$idTransaksiBeli','$simpan[barcode]',
                    '$simpan[tglExpire]',$simpan[qty],$simpan[qty],'$simpan[hargaBeli]','$_SESSION[uname]','$simpan[kd_brg]','$simpan[idBarang]')";
$sqlgerakan = "INSERT INTO pergerakan (barcode,qtymasuk,tanggal) VALUES('$simpan[barcode]',$simpan[qty],'$tgl')";

      mysql_query($sql_simpan) or die(mysql_error());
        mysql_query($sqlgerakan) or die(mysql_error());
...
// Jika jika qty "terkena" harga banded, maka harga_jual_asli diisi
      ...
      if (!$transferahad) {
         $sql = "INSERT INTO detail_jual(idTransaksiJual,idBarang, barcode,
	                        jumBarang,hargaJual,harga_jual_asli,admin, nomorStruk, hargaBeli)
							  VALUES({$id_transaksi},{$simpan['idBarang']}, '{$simpan['barcode']}',
							  {$simpan['jumBarang']},{$simpan['harga']},{$hargaJualAsli},'{$_SESSION['uname']}', {$NomorStruk}, {$simpan['hargaBeli']})";
                $sqlgerakan = "INSERT INTO perkerakan (barcode,qtykeluar,tanggal) VALUES('$simpan[barcode]','$simpan[qty]','$tgl')";

         mysql_query($sql) or die('Gagal simpan transaksi detail'.$sql.' '.mysql_error());
           mysql_query($sqlgerakan) or die('Gagal simpan transaksi detail '.$sqlgerakan.''.mysql_error());
...
// hapus transaksi jual ini dari table tmp_detail_jual
      ...
      releaseCustomer();
         ...
         header('location:./js_jual_barang.php?act=caricustomer');
	  }}
   ...
   if (isset($_POST['username'])) {
      ...
      // $pass = md5($_POST['pass']);
      $pass = $_POST['pass'];
      $sql = "select admin_id    from admin    where username='{$username}' and password='{$pass}'";
   ...
   if ($_SESSION['hakAdmin'] || $transferahad) {
      ...
      header('location:./js_jual_barang.php?act=caricustomer&bg=1');
   }
}  
elseif ($module == 'diskon' && $act == 'cariuser') {
   if (isset($_POST['uname'])) {
      $username = htmlspecialchars($_POST['uname']);
      
      $sql = "select username from user  where username='{$username}'";
      $result = mysql_query($sql) or die(mysql_error());
      $ketemu = mysql_fetch_array($result);
      if ($ketemu) {
...
elseif ($module == 'diskon' && $act == 'update') {
   if (isset($_POST['username'])) {
      $username = htmlspecialchars($_POST['username']);
...
elseif ($module == 'diskon' && $act == 'diskonk') {
   if (isset($_POST['diskonk'])) {
      $newdiskon = $_POST['diskonk'];
      $idCustomer = $_POST['userid'];
      
      $sql = "update user
          set diskonkhusus='{$newdiskon}' where user_id = $idCustomer ";
      $result = mysql_query($sql) or die(mysql_error());
...
elseif ($module == 'penjualan_barang' && $act == 'inputmember') {
   if (isset($_POST['username'])) {
      $username = htmlspecialchars($_POST['username']);
      $nmdepan =  htmlspecialchars($_POST['nm_depan']);
      $nmbelakang = htmlspecialchars($_POST['nm_belakang']);
      $alamat = htmlspecialchars($_POST['alamat']);
      $kecamatan = htmlspecialchars($_POST['kecamatan']);
      $kota = htmlspecialchars($_POST['kota']);
      $provinsi = htmlspecialchars($_POST['provinsi']);
      $kdpos = htmlspecialchars($_POST['kd_pos']);
      $hp = htmlspecialchars($_POST['noHP']);
      
      $sql = "insert into user (username,firstname,lastname,alamat,kecamatan,kota,provinsi,kode_pos,hp, password,level_id) values ('{$username}','{$nmdepan}','{$nmbelakang}','{$alamat}','{$kecamatan}','{$kota}','{$provinsi}','{$kdpos}','{$hp}','asdwqe123','0')";
      $result = mysql_query($sql) or die(mysql_error());
