...
<form method="post" action="<?php echo $caller; ?>.php?act=caricustomer&action=tambah">
   ...
   ?>
   <?php
   $sql = "SELECT * FROM barang LEFT JOIN size ON barang.size_id = size.size_id LEFT JOIN warna ON barang.warna_id = warna.warna_id WHERE kd_brg LIKE '%".$_POST[namabarang]."%' ORDER BY kd_brg ASC";
   ...
   <table class="tabel" style="width:100%">
      ...
      while ($data = mysql_fetch_array($query)) :
         ...
         <tr class="<?php echo $no % 2 === 0 ? 'alt' : ''; ?> <?php echo $nonAktif ? 'merah' : '' ?>">
            ...
            <td class="right"><?php echo $data['qty']; ?></td>
            <td class="right"><?php echo number_format($data['harga'], 0, ',', '.'); ?></td>
