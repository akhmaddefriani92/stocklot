...
<div class="mainpanel">
    ...
    <table id="basicTable" class="table table-striped table-bordered responsive">
        ...
        if (isset($_POST["cari"])) { 
            ...
            $sql = "SELECT * FROM `transaksijual` LEFT JOIN `detail_jual` ON `transaksijual`.`idTransaksiJual` = `detail_jual`.`idTransaksiJual` LEFT JOIN `barang` ON `detail_jual`.`idBarang`=`barang`.`barang_id` LEFT JOIN `size` ON `barang`.`size_id` = `size`.`size_id` LEFT JOIN `user` ON `transaksijual`.`user_id` = `user`.`user_id` LEFT JOIN `level` ON `user`.`level_id` = `level`.`level_id` WHERE `transaksijual`.`tglTransaksiJual` BETWEEN '$tanggalawal%' AND '$tanggalakhir%' ORDER BY `detail_jual`.`uid`";
            ...
            $total ="SELECT (SUM(hargaJual * jumBarang)) as totaljual, (SUM(hargaBeli * jumBarang)) as totalbeli, (SUM(jumBarang)) as totalbarang FROM `transaksijual` LEFT JOIN `detail_jual` ON `transaksijual`.`idTransaksiJual` = `detail_jual`.`idTransaksiJual` LEFT JOIN `barang` ON `detail_jual`.`idBarang`=`barang`.`barang_id` LEFT JOIN `size` ON `barang`.`size_id` = `size`.`size_id` LEFT JOIN `user` ON `transaksijual`.`user_id` = `user`.`user_id` LEFT JOIN `level` ON `user`.`level_id` = `level`.`level_id` ";
