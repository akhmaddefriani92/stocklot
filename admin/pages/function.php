...
function findCustomer($username) {
   $query = mysql_query("SELECT * from user LEFT JOIN level ON user.level_id = level.level_id WHERE username = '$username'");
   ...
   $_SESSION[alamat] = $dataCustomer[alamat];
    ...
    $_SESSION[nohp] = $dataCustomer[hp];
    $_SESSION['diskonkhusus'] = $dataCustomer['diskonkhusus'];
   $_SESSION['customerDiskonP'] = $dataCustomer['diskon_persen'];
   $_SESSION['customerDiskonR'] = $dataCustomer['diskon_rupian'];
...
function cekBarang($kdbrg) {
   ...
   $sql = "SELECT b.kd_brg,b.barang_id, b.nm_brg,b.size_id,size.size, b.harga, b.barcode, d.hargaBeli, b.qty FROM detail_beli AS d, barang AS b LEFT JOIN size ON b.size_id = size.size_id WHERE b.kd_brg = '$kdbrg' AND d.kd_brg = '$kdbrg' ORDER BY d.barang_id DESC LIMIT 1 ";
   //echo $sql;
   $query = mysql_query($sql) ;
   $data = mysql_fetch_array($query);

   //HS jika tidak ada data yang ditemukan - mungkin baru ada di tabel barang, tapi belum ada di cekba
   if (mysql_num_rows($query) < 1) {
      $sql = "SELECT barang.kd_brg, barang.barang_id, barang.nm_brg, barang.harga, barang.barcode,barang.qty FROM barang
	 
		WHERE kd_brg = '$kdbrg' LIMIT 1";
      $query = mysql_query($sql);
      $data = mysql_fetch_array($query);
   };
...
function cekBarangJual($barcode) {
   // jika ada banyak barang dengan barcode yang sama, kembalikan record yang terbaru
   $sql = "SELECT b.kd_brg,b.barang_id, b.nm_brg,b.size_id,size.size, b.harga, b.barcode, d.hargaBeli, b.qty FROM detail_beli AS d, barang AS b LEFT JOIN size ON b.size_id = size.size_id WHERE b.barcode = '$barcode' AND d.barcode = '$barcode' ORDER BY d.barang_id DESC LIMIT 1 ";
   //echo $sql;
...
function cekBarangTemp($idSupplier, $kdbrg,$size) {
   $adaBeli = 0;
   $cek = mysql_query("SELECT * from tmp_detail_beli where supplier_id = '$idSupplier' and kd_brg = '$kdbrg' and size_id = '$size'")or die (mysql_error());
...
function ubahJumlahBarangBeliTemp($idSupplier,$jumlah, $kdbrg, $size) {
   mysql_query("UPDATE tmp_detail_beli SET qty = '$jumlah'
            WHERE supplier_id = '$idSupplier' and kd_brg = '$kdbrg' and size_id = '$size' ") or die(mysql_error());
}
