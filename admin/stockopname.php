...
<div class="row">
                         ...
                         <?php include "config.php";
                               ...
                               if (empty($_GET)) { $sql = "SELECT barang.kd_brg, barang.barcode, barang.nm_brg,barang.size_id,size.size,warna.warna,merk.nama,detail_beli.hargaBeli,fast_stock_opname.jmlTercatat FROM barang 
                                ...
                                LEFT JOIN fast_stock_opname ON barang.barang_id = fast_stock_opname.barang_id AND fast_stock_opname.tanggalSO = '$tanggal'
                                ...
                                <tbody> <?php while ($r = mysql_fetch_array($query)) {  ?>
                                  <tr class="<?php echo $no % 2 === 0 ? 'alt' : ''; ?>">
                                    <td><?php echo $no; ?></td>
                                      ...
                                      <form method="POST" action="" >  <input type="hidden" name="barcode" value="<?php echo $r['barcode']; ?>"> <input type="hidden" name="tanggal" value="<?php echo $tanggal; ?>">
                                      <td><input type="text" name="qty"  ></td>
                                       <td><button name="tambahso" type="submit">input</button></td></form>
</tr> <?php $no++;
}
?>
                               <?php if (isset($_POST['tambahso'])) {
                               
                                
                                 $barcode = $_POST['barcode'];
                                $tgl = $_POST['tanggal'];
                                $qty = $_POST['qty'];
                                $query1 = mysql_query("SELECT * FROM barang WHERE barcode = '$barcode'");
                                $b = mysql_fetch_array($query1);
                                $id = $b['barang_id'];
                                $nm_brg = $b['nm_brg'];
                                $qtyb = $b['qty'];
                                $selisih = $qty - $qtyb;
                                $query2 = mysql_query("SELECT * FROM fast_stock_opname WHERE barcode ='$barcode' AND tanggalSO = '$tgl'");
if (mysql_num_rows($query2)>0){
 mysql_query("UPDATE fast_stock_opname SET jmlTercatat = '$qty', selisih = '$selisih' WHERE barcode = '$barcode' AND tanggalSO = '$tgl'");
    header("Refresh:0");
} else {
   
    mysql_query("INSERT INTO fast_stock_opname (barang_id,tanggalSO,username,barcode,namaBarang,jmlTercatat,selisih,approved) VALUES ('$id','$tgl','admin','$barcode','$nm_brg','$qty','$selisih','0')");
header("Refresh:0");
}
                                } ?> 
                            </table>
